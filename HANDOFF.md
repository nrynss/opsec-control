...
| **P5** | ✅ **Grok Builder (2026-06-29)** | `internal/api` | `POST /perception` (multipart or raw bytes → `Perception.Interpret` → publish events to bus). Inject a `Perception` dependency into `Server`. Handler stamps sim time for validity + publishes to bus. Tests + isolation green. | P1 ✅, P2 |
